...
def run_all(rule_list, defined_variables, stop_on_first_trigger=False):
    rule_was_triggered = False
    for rule in rule_list:
        result = run(rule, defined_variables)
        if result:
...
def run(rule, defined_variables):
    rule_triggered = check_conditions_recursively(rule, defined_variables)
    if rule_triggered:
        return True
    return False
